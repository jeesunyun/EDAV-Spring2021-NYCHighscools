...
hs_join.2019 <- hs_dir_tidy.2019 %>% 
  ...
  mutate(pct_Poverty = as.numeric(pct_Poverty)*0.01)
...
Column `Total_Emrollment` comes from Demographics data, and `total_students` come from Directory data.
```{r}
hs_join.2019[which(is.na(hs_join.2019["Total_Enrollment"])), ]
```

```{r}
hs_join.2019[which(is.na(hs_join.2019["Total_Enrollment"])), c('school_name', 'grades2018', 'finalgrades')]
```
...
gr6_to_gr12 <- hs_final[which(hs_final$grades2018 == "6 to 12"), ] %>% 
  dplyr::select(-c(neighborhood, hs_enroll, non_hs, PK, Grade_K, Grade_1, Grade_2, Grade_3, Grade_4, Grade_5, finalgrades, )) %>% 
  

gr9_to_gr12 <- hs_final[which(hs_final$grades2018 == "9 to 12"), ]
```
